    ...
    >
      ...
      >
        ...
        {group
          .filter(
            post =>
              post.node.title.length > 0 && post.node.featured != 'featured'
          )
          .map(({ node: post }, index) => {
              ...
              <div key={post.id}>
                <BlogCard image={post.featuredImage ? true : false}>
                  <Link to={`/${pathPrefix}/${post.slug}`}>
                    {image ? (
                      <Img alt={post.featuredImage.title} resolutions={image} />
                    ) : null}
                    <div css="padding: 24px;">
                      <div>
                        ...
                        <span className={dateStyle}>
                          {' '}
                          {post.updatedAt} &middot;{' '}
                        </span>
                        <StyledSpan>
                          {post.blog.childMarkdownRemark.timeToRead} min read{' '}
                        </StyledSpan>
                      </div>
                      <h3>{post.title}</h3>
